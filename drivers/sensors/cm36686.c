...
static int cm36686_parse_dt(struct device *dev, struct  cm36686_platform_data *pdata)
{
	...
	pdata->default_hi_thd = 34;
	pdata->default_low_thd = 30;
	pdata->cancel_hi_thd = 34;
	pdata->cancel_low_thd = 30;
	return 0;
}
#else
...
static void prox_regulator_onoff(int en)
{
	int rc;
	static struct regulator* ldo19;
	static struct regulator* lvs1;
	static struct regulator* ldo23;
	printk(KERN_ERR "%s %s\n", __func__, (en) ? "on" : "off");
	...
	if(!ldo23){
		ldo23 = regulator_get(NULL,"8226_l23");
		rc = regulator_set_voltage(ldo23,3300000,3300000);
		pr_info("[TMP] %s, %d\n", __func__, __LINE__);
		if (rc){
			printk(KERN_ERR "%s: taos set_level failed (%d)\n",__func__, rc);
		}
	...
	if(en){
		...
		rc = regulator_disable(lvs1);
		if (rc){
			printk(KERN_ERR "%s: taos disable failed (%d)\n",__func__, rc);
		}

		rc = regulator_disable(ldo23);
		if (rc){
			printk(KERN_ERR "%s: taos disable failed (%d)\n",__func__, rc);
		}
	}
