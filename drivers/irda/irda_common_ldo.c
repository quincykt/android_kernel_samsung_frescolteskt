...
static int irda_fw_update(struct ir_remocon_data *ir_data)
{
	...
	int i=0, k, ret, checksum, frame_count = 0, start_addr;
	u8 buf_ir_test[8];
	const u8 *IRDA_fw;
	ret = 0;

	/* Switch On IRDA in user IR mode (wake high) and read FW */
	irda_led_onoff(1);
	data->pdata->ir_wake_en(data->pdata, 1);
...
fix_fw:
	...
	/* Expected sequence in bootmode = 0x00 ,0x01 ,0x3F ,0xFF ,0x10 ,0x00 ,0x01 ,0x4F */
	ret = i2c_master_recv(client, buf_ir_test, MC96_READ_LENGTH);
	if (ret < 0)
		printk(KERN_ERR " %s: err %d\n", __func__, ret);
	...
	if (ret == checksum)
		printk(KERN_INFO "%s: Boot mode, First check for FW download clear! ret=%04x\n", __func__, ret);
	else {
		printk(KERN_ERR "ABOV IC bootcode broken: First check failed\n");
		goto fw_update_end;
	}
	...
	if (start_addr == 0x3FFF)
		...
		pr_info("%s: ABOV IC bootcode broken: Second check failed checksum = %4x\n", __func__, start_addr);
		goto fw_update_end;
	}
	msleep(30);
...
fw_update_end:
	...
	irda_led_onoff(0);
	msleep(100);
	/* Power on in user IR mode and read FW again *
	 * Correct 1.3 FW read = 0x00 ,0x06 ,0x01 ,0x03 ,0x01 ,0x06 ,0x00 ,0x11
	 * Correct 1.1 FW read = 0x00 ,0x06 ,0x01 ,0x01, 0x01 ,0x06 ,0x00 ,0x0F
	 */
	irda_led_onoff(1);
	gpio_set_value(data->pdata->irda_poweron, 1);
	data->pdata->ir_wake_en(data->pdata, 1);
	...
	if((update_retry_count == MAX_UPDATE_RETRY - 1) && (ret == 0x101)) {
		download_pass = 1;
